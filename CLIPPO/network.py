...
class CLIPPO(nn.Module): 
    def __init__(self): 
        ...
        self.encoder = nn.Sequential(*[nn.Conv2d(3, 32, 3), nn.ELU(), nn.Conv2d(32, 64, 3), nn.ELU(), nn.Flatten(), nn.AdaptiveMaxPool1d(124), nn.Linear(124, 512)])
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        projection_input = 512#768
        # 4 x 64 = 256
        #self.text_proj = nn.Sequential(*[nn.Linear(projection_input, 256), nn.ReLU(), nn.Linear(256, 2)])
        self.image_proj = nn.Sequential(*[nn.Linear(projection_input, 256), nn.ELU(), nn.Linear(256, 64),nn.ELU(),nn.Linear(64,2)])
